...
def render_features():
    ...
    fig = px.imshow(hm, color_continuous_scale="RdYlGn_r", labels=dict(color="Avg severity"),
                    aspect="auto", template=TPL, height=420, title="Average severity by sector and year")
    ...
    with c1:
        ...
        fig = px.imshow(corr, text_auto=True, color_continuous_scale="RdBu", zmin=-1, zmax=1,
                        aspect="auto", template=TPL, height=420, title="How the engineered signals relate")
    ...
    with c2:
        fig = px.histogram(filtered, x="Impact_Score", color="Incident_Category", barmode="overlay", opacity=0.75,
                           color_discrete_map=CAT_COLOURS, nbins=6, template=TPL, height=420,
                           title="Breach scale, cyber vs non-cyber",
                           labels={"Impact_Score": "Impact score (1 = 1–9 people … 6 = 100k+)"})
    ...
    with c3:
        ...
        fig = px.bar(ds, x="Severity_Score", y="Decision_Taken", orientation="h", color="Severity_Score",
                     color_continuous_scale="RdYlGn_r", template=TPL, height=400, title="Severity behind each ICO decision")
        fig.update_layout(coloraxis_showscale=False, yaxis=dict(categoryorder="total ascending"),
                          xaxis_title="Avg severity", yaxis_title="")
    ...
    with c4:
        ...
        if len(dual) > 0:
            ...
            fig = px.bar(dh, x="Breaches", y="Sector", orientation="h",
                         ...
                         title="Cyber + special-category exposure")
    ...
    fig = px.treemap(tm, path=["Sector", "Incident_Category", "Decision_Taken"], values="Reports",
                     color="Reports", color_continuous_scale="Purples", template=TPL, height=480,
                     title="Sector → category → outcome")
