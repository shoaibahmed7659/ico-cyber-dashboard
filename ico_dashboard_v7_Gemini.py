...
div[data-testid="stPopover"]{ position:fixed; bottom:1.4rem; right:1.4rem; z-index:1000; }
div[data-testid="stPopover"] button{ border-radius:999px; box-shadow:0 6px 18px rgba(79,70,229,.32);
  background:var(--accent); color:#fff; border:none; font-weight:600; padding:.55rem 1rem; }
</style>
...
def render_model():
    ...
    st.session_state["model_summary"] = (
        "best model = {} (3-fold CV ROC-AUC {:.3f}); incident type is excluded to avoid target leakage; "
        "trained on a stratified sample of {:,} of {:,} records.".format(
            best_name, model_results[best_name]["cv_auc_mean"],
            model_results[best_name]["train_n"], model_results[best_name]["full_n"]))
...
# ════════════════════════════════════════════════════════════════════════════════
#  ASSISTANT  (context-aware Gemini chat — floating)
# ════════════════════════════════════════════════════════════════════════════════
SECTION_GUIDE = {
    "Overview": "headline figures (total reports, cyber share, high-impact %, special-category %, average severity, "
                "high-risk sector count), live auto-insights, a cyber vs non-cyber trend line, the category split, "
                "and the annual cyber-rate chart.",
    "Trends": "reporting volume over time (by year or quarter), cyber's share as a fraction, average severity over "
              "time, and the most common breach types over time.",
    "Sectors": "report volume per sector split by cyber/non-cyber, the cyber rate per sector coloured by a "
               "volume-adjusted risk tier, and a per-sector drill-down of breach types and data categories.",
    "Impact & Risk": "breach size by people-affected band (cyber vs non-cyber) and the spread of severity scores "
                     "across the busiest sectors.",
    "Feature Insights": "average severity by sector and year (heatmap), how the engineered measures correlate, breach "
                        "scale by impact band, average severity behind each ICO decision, the cyber + special-category "
                        "exposure leaders, and a sector→category→outcome treemap.",
    "Modelling": "a classifier predicting cyber vs non-cyber from breach characteristics (incident type withheld to "
                 "avoid target leakage); accuracy/precision/recall, test and 3-fold cross-validated ROC-AUC, ROC curve, "
                 "confusion matrix, permutation importance, a model comparison table and a recommendation.",
    "Risk Predictor": "an interactive estimator: choose breach characteristics and the best model returns a cyber "
                      "probability with a severity score.",
    "Data & Method": "data completeness, reports per year, regulatory outcome by category, the data dictionary, the "
                     "engineered-feature definitions, the modelling method, limitations and a filtered-data download.",
}

SYSTEM_PROMPT = (
    "You are the built-in assistant for the ICO Data Breach Intelligence dashboard, which analyses personal-data "
    "breaches self-reported to the UK Information Commissioner's Office (2019–Q4 2025). Help the user understand the "
    "charts, the figures currently on screen, the ICO, UK GDPR and data protection generally. Rules: use ONLY the "
    "numbers given in CONTEXT and never invent statistics; if something is not in CONTEXT, say so briefly and point to "
    "the section that would show it. Keep answers short and plain-English, with British spelling. Do not give legal "
    "advice; you may explain regulation in general terms."
)

def build_context():
    f = filtered
    sec = st.session_state.get("active_section", "Overview")
    lines = ["The user is viewing the '" + sec + "' section, which shows: " + SECTION_GUIDE.get(sec, "")]
    lines.append("Active filters — years: " + (", ".join(map(str, sorted(year_sel))) if year_sel else "none") +
                 "; sectors: " + str(len(sector_sel)) + " of " + str(len(sectors)) + " selected; categories: " +
                 (", ".join(cat_sel) if cat_sel else "none") + ".")
    if not f.empty:
        lines.append(("Figures for the current selection — reports: {:,}; cyber share: {:.1f}%; high-impact (1,000+): "
                      "{:.1f}%; special-category: {:.1f}%; average severity: {:.2f}/11; sectors in view: {}.").format(
                      len(f), f["Is_Cyber"].mean() * 100, f["Is_High_Impact"].mean() * 100,
                      f["Is_Special_Category"].mean() * 100, f["Severity_Score"].mean(), f["Sector"].nunique()))
        lines.append("Auto-insights on screen: " + " ".join(re.sub("<[^>]+>", "", x) for x in insights(f)))
    if st.session_state.get("model_summary"):
        lines.append("Modelling results: " + st.session_state["model_summary"])
    return "CONTEXT\n" + "\n".join(lines)
...
def ask_gemini(history, key):
    ...
    return (getattr(resp, "text", "") or "").strip() or "I couldn't generate a reply — please try rephrasing."

def _friendly_error(e):
    m = str(e).lower()
    if any(t in m for t in ["429", "quota", "rate", "resource_exhausted", "exhausted"]):
        return "The free Gemini quota is busy right now (rate limit). Wait a few seconds and try again."
    if "not found" in m or "not supported" in m or ("model" in m and "invalid" in m):
        return ("That Gemini model name looks unavailable on your key. Set a current one via **GEMINI_MODEL** in "
                "secrets (any current Flash model).")
    if any(t in m for t in ["api key", "api_key", "permission", "401", "403", "unauthenticated"]):
        return "The Gemini key was rejected — check **GEMINI_API_KEY** in your secrets."
    return "Sorry — the assistant hit a problem reaching Gemini. Please try again in a moment."

@st.fragment
def render_chat():
    with st.popover("💬  Ask the data"):
        st.markdown("**Assistant** — ask about the charts, the figures on screen, or the ICO.")
        key = _gemini_key()
        if not key:
            st.info("Add a free Gemini key to switch this on: put `GEMINI_API_KEY = \"…\"` in "
                    "`.streamlit/secrets.toml` locally and in the app's **Settings → Secrets** on Streamlit Cloud.")
            return
        try:
            import google.genai  # noqa: F401
        except Exception:
            st.warning("The assistant needs the **google-genai** package — it's in requirements.txt; locally run "
        ...
        with st.form("chat_form", clear_on_submit=True):
            q = st.text_input("Your question", label_visibility="collapsed",
                              placeholder="e.g. Which sector has the highest cyber rate?")
            sent = st.form_submit_button("Send", width="stretch")

        if sent and q.strip():
            hist.append({"role": "user", "text": q.strip()})
            with st.chat_message("user"):
                st.markdown(q.strip())
            with st.chat_message("assistant"):
                with st.spinner("Thinking…"):
                    try:
                        ans = ask_gemini(hist, key)
                    except Exception as e:
                        ans = _friendly_error(e)
                st.markdown(ans)
            hist.append({"role": "assistant", "text": ans})

        if hist and st.button("Clear", key="chat_clear"):
            st.session_state["chat"] = []
            st.rerun(scope="fragment")

# ════════════════════════════════════════════════════════════════════════════════
#  NAVIGATION  (stateful — only the active section renders, so the app stays light)
# ════════════════════════════════════════════════════════════════════════════════
SECTIONS = ["Overview", "Trends", "Sectors", "Impact & Risk",
            "Feature Insights", "Modelling", "Risk Predictor", "Data & Method"]
RENDERERS = {"Overview": render_overview, "Trends": render_trends, "Sectors": render_sectors,
             "Impact & Risk": render_impact, "Feature Insights": render_features,
             "Modelling": render_model, "Risk Predictor": render_predictor, "Data & Method": render_data}
...
render_chat()                # floating assistant (CSS-pinned bottom-right)
RENDERERS[section]()         # render only the chosen section
