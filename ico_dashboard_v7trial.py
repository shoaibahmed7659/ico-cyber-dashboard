...
T_OVERVIEW, T_TRENDS, T_SECTORS, T_IMPACT, T_MODEL, T_PREDICT, T_DATA = st.tabs(
    ["Overview", "Trends", "Sectors", "Impact & Risk", "Modelling", "Risk Predictor", "Data & Method"])
...
with T_OVERVIEW:
    if filtered.empty:
        ...
        r1 = st.columns(4)
        r1[0].markdown(kpi("Breach reports", f"{total:,}", "Self-reported to the ICO"), unsafe_allow_html=True)
        r1[1].markdown(kpi("Cyber share", str(p_cyber) + "%", "Malicious / technical origin", "red"), unsafe_allow_html=True)
        ...
        with a:
            ...
            st.plotly_chart(fig, width="stretch", key="ov_line")
        ...
        with b:
            ...
            st.plotly_chart(fig, width="stretch", key="ov_bar")
        ...
        st.plotly_chart(fig, width="stretch", key="ov_yoy")
...
def render_trends():
    ...
    st.plotly_chart(fig, width="stretch", key="tr_bar")
    ...
    with c2:
        ...
        st.plotly_chart(fig, width="stretch", key="tr_sev")
    ...
    if not td.empty:
        ...
        st.plotly_chart(fig, width="stretch", key="tr_types")
...
def render_sectors():
    ...
    with c1:
        ...
        st.plotly_chart(fig, width="stretch", key="se_stack")
    ...
    with c2:
        ...
        st.plotly_chart(fig, width="stretch", key="se_rate")
    ...
    with d1:
        ...
        fig.update_layout(yaxis=dict(categoryorder="total ascending"))
        st.plotly_chart(fig, width="stretch", key="se_types")
    ...
    with d2:
        ...
        fig.update_layout(yaxis=dict(categoryorder="total ascending"))
        st.plotly_chart(fig, width="stretch", key="se_data")
...
with T_IMPACT:
    if filtered.empty:
        ...
        eyebrow("How bad", "Impact & severity")
        ...
        st.plotly_chart(fig, width="stretch", key="im_bands")
        ...
        with c1:
            ...
            fig = px.box(filtered[filtered["Sector"].isin(top12)], x="Sector", y="Severity_Score",
                         color="Incident_Category", color_discrete_map=CAT_COLOURS, template=TPL, height=430)
            fig.update_layout(xaxis_tickangle=-35, yaxis_title="Severity (0–11)", xaxis_title="")
            st.plotly_chart(fig, width="stretch", key="im_box")
        ...
        with c2:
            corr_cols = [c for c in ["Is_Cyber", "Is_High_Impact", "Impact_Score",
                                     "Is_Special_Category", "Within_72hrs", "Severity_Score"]
                         if c in filtered.columns]
            corr = filtered[corr_cols].corr().round(2)
            fig = px.imshow(corr, text_auto=True, color_continuous_scale="RdBu", zmin=-1, zmax=1,
                            aspect="auto", template=TPL, height=430)
            st.plotly_chart(fig, width="stretch", key="im_corr")
            cap("How the engineered numeric signals move together (+1) or apart (−1). Severity correlates with its own "
                "inputs by design; treat the rest as association, not cause.")

        eyebrow("Highest exposure", "Cyber breaches involving special-category data")
        dual = filtered[(filtered["Is_Cyber"] == 1) & (filtered["Is_Special_Category"] == 1)]
        if len(dual) > 0:
            dh = dual["Sector"].value_counts().head(12).reset_index(); dh.columns = ["Sector", "Breaches"]
            fig = px.bar(dh, x="Breaches", y="Sector", orientation="h",
                         color_discrete_sequence=[C_PURPLE], template=TPL, height=400)
            fig.update_layout(yaxis=dict(categoryorder="total ascending"))
            st.plotly_chart(fig, width="stretch", key="im_dual")
            cap("Sectors with the most breaches that are <i>both</i> cyber and involve special-category data — "
                "the highest-exposure combination for regulatory action.")
        else:
...
def render_model():
    ...
    with c1:
        ...
        st.plotly_chart(fig, width="stretch", key="md_roc")
    ...
    with c2:
        ...
        st.plotly_chart(fig, width="stretch", key="md_cm")
    ...
    if r.get("perm_importance") is not None:
        ...
        st.plotly_chart(fig, width="stretch", key="md_perm")
...
def render_predictor():
    ...
    if st.button("Estimate cyber probability", width="stretch", type="primary"):
        ...
        with g:
            ...
            st.plotly_chart(fig, width="stretch", key="pr_gauge")
        with t:
...
with T_DATA:
    ...
    st.markdown(
        ...
        "executive summary, **Trends / Sectors / Impact & Risk** are the exploratory analysis, **Modelling** explains "
    ...
    with c2:
        ...
        st.plotly_chart(fig, width="stretch", key="da_year")
    ...
    st.plotly_chart(fig, width="stretch", key="da_dec")
